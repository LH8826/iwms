...
class MailUtils():
    ...
    def send_test_report(cls):
        ...
        # 接收测试报告的邮箱，可以不止一个,严格按照以下写法，否则容易报错！
        receiver = ["yanping@hd123.com", "haidingtest@sina.com"]
        str_add = ','.join(receiver)
        print(str_add)
        auto_code = "467bffc1f4451677"
        subject = "YSApp_report"

        # 在测试报告存放的目录里，找到最新的测试报告文件
        lists = os.listdir('D://HDiWMS//report_of_day//YSApp//')  # 列出目录的下所有文件和文件夹保存到lists
        lists.sort(key=lambda fn: os.path.getmtime('D://HDiWMS//report_of_day//YSApp//' + fn))  # 按时间排序
        file_new = os.path.join('D://HDiWMS//report_of_day//YSApp//', lists[-1])
